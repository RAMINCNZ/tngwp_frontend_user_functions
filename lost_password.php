...
function tg_validate_url() {
	global $post;
	$page_url = esc_url(plugins_url( 'tngwp_frontend_user_functions/lost_password.php' ));
...
if (!$user_ID) { //block logged in users
	...
	if(isset($_GET['key']) && $_GET['action'] == "reset_pwd") {
		...
		if(!empty($reset_key) && !empty($user_data)) {
			...
			$message .= __('You can now login with your new password at: ') . get_bloginfo('wpurl')."/wp-content/plugins/tngwp_frontend_user_functions/login.php" . "\r\n\r\n";
			...
			if ( $message && !wp_mail($user_email, 'Requested Temporary Password', $message) ) {
				...
				$redirect_to = plugins_url( 'tngwp_frontend_user_functions/reset.php' )."?action=reset_success";
...
?>
		...
		<div id="primary" class="content-area col-sm-9 clearfix">
